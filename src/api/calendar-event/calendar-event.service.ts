...
export class CalendarEventService {
  ...
  public async create(calendarEventCreateRequest: CalendarEventCreateRequestDto) {
    const { images, plantId, ...restCalendarEventCreateRequestDto} = calendarEventCreateRequest;
    ...
    const calendarEvent = await this.calendarEventRepository.save({ ...restCalendarEventCreateRequestDto, plant });
  ...
  public async update(id: number, calendarEventUpdateRequest: CalendarEventUpdateRequestDto) {
    const oldCalendarEvent = await this.getById(id)
    const updateResult = await this.calendarEventRepository.update(id, calendarEventUpdateRequest);
  ...
  private async getAllByPlant({
    plantId
                              }: {
    plantId: number, startAt?: Date, endAt?: Date
  }) {
  ...
  private async getById(id: number) {
    const calendarEvent = await this.calendarEventRepository.findOne({ where: { id }, relations: { pauses: true } });
